...
public class MapUtil {
     ...
     *
     * @param keyPath 要读取的目标路径
     * @param map json或yaml转换成的map文件，或者多层嵌套的map,key类型必须为String
     * @return 读取的对象
     */
    public static Object readMapByPoint(Map<String,?> map, String keyPath){
     ...
     * 可以用'.'的方式读取修改map
     * @param map json或yaml转换成的map文件，或者多层嵌套的map，key类型必须为String
     * @param keyPath 要读取的目标值
     * @param value 要修改的值
     */
    public static void setMapByPoint(Map<String,?> map, String keyPath, Object value){
        String[] keys = keyPath.split("\\.");
        Map endMap = map;
        for(int i = 0; i < keys.length-1 && endMap != null; i++){
            endMap = (Map) endMap.get(keys[i]);
        ...
        if(endMap != null){
            endMap.replace(keys[keys.length - 1],value);
        }
    }
